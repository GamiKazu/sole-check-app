...
def analyze_images(
    ...
    overall_shape, shape_conf = unify_shapes(
        shape_results
    )


    if (
        representative_rgb is None
        or
        representative_mask is None
    ):
...
""",
    ...
    result_card(
        "1. 足の形",
        (
            f"<span class='result-main'>{shape}</span>"
            "<br><br>"
            f"{shape_text}"
        ),
        "card-beige"
